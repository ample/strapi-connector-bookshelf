...
module.exports = function createQueryBuilder({ model, strapi }) {
  ...
  function find(params, populate, { transacting } = {}) {
    ...
    // -------------------------------------------- ###
    // When filtering by ID cast invalid input values as a number 
    if(Object.keys(filters).includes('where')) {
      filters.where = filters.where.map(f => {
        if(f.field == 'id' && typeof f.value == 'object') f.value = Number(f.value) || 0
        return f
      })
    }
    // -------------------------------------------- ###
